...
/*
__global__ 
void gaussianBlur_shared_v2(unsigned char *d_in, unsigned char *d_out, const int rows, const int cols, float *d_filter, const int filterWidth){
  ...
  //all theads read in a pxiel
  //but not all participate in the calculation


  // per thread, read in your pixel into shared memory


  // if you are a thread working on a pixel minus filter apron, then ** calculate blur** as below

  // if not then do nothing

  // sync threads

  // then read in next chunk of shared memory, so we have the bits in between


  // then calcualte the last bits of blurr in between
  ...
  int shared_row_id, shared_col_id;
  int global_row_id, global_col_id;

  for (row_offset = -filterRadius; row_offset <= filterRadius; row_offset++){
    for (col_offset = -filterRadius; col_offset <= filterRadius; col_offset++){
      // calculate offsets
      shared_row_id = shared_row + row_offset;
      shared_col_id = shared_col + col_offset;
      global_row_id = global_row + row_offset;
      global_col_id = global_col + col_offset;
      
      // check bounds
      if (global_row_id < rows && global_col_id < cols && global_row_id >= 0 && global_col_id >= 0){
        shared_offset = (shared_row + row_offset) * TILE_WIDTH + (shared_col + col_offset);
        if (shared_offset >= 0 && shared_offset < TILE_WIDTH*TILE_WIDTH){
          float pixel_value = (float)local_in[shared_offset];
          float filter_value = d_filter[filter_offset]; 
